...
datos=pd.read_csv("https://raw.githubusercontent.com/Albertuff/Machine-Learning/master/datos/estatura_peso.csv")

# Definimos los atributos(features) y las variable objeivo(target)

X=datos[["height","weight"]]           # Atributos
Y=datos["gender"]                      # Respuesta
...
cv_repetida=RepeatedKFold(n_splits=5,n_repeats=100)

rejilla=GridSearchCV(Modelo,param_grid=espacio_parametros,cv=cv_repetida,n_jobs=-1).fit(X,Y)  # El primer argumento es el modelo a entrenar, el segundo es el espacio donde están de los hiperparámetros,
...
cv_interna=RepeatedKFold(n_splits=5,n_repeats=100)
...
modelo_estrella=KNeighborsClassifier(n_neighbors=9).fit(X,Y)
